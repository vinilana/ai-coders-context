...
export class IncrementalDocumentationGenerator {
  ...
  async updateDocumentation(
    ...
    for (const module of affectedModules) {
      try {
        await this.updateModuleDocumentation(module, repoStructure, docsDir, verbose);
        updated++;
      } catch (error) {
        if (verbose) {
    ...
    await this.cleanupEmptyModules(docsDir, repoStructure, verbose);
...
- **Branch**: ${branch}
- **Generated**: ${new Date().toISOString()}
- **Last Processing**: ${state?.lastRun || 'First run'}
- **Files Processed**: ${state?.processedFiles?.length || 0}
