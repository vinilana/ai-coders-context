...
## Purpose
You are an AI assistant responsible for refreshing the documentation (`docs/`), the agent index (`agents/index.json`), and individual agent playbooks (`agents/*.json`) generated by the ai-context scaffolding tool. Your goal is to bring every guide up to date with the latest repository state, fill in placeholder sections, and maintain cross-references between docs, the agent roster, and test plans.
...
4. **Agent Playbook Alignment**
   ...
   - Reconcile `agents/index.json` so the agent roster, responsibilities, and guidance reflect your updates.

5. **Test Plan Refresh**
   - Update `test-plan.json` with the latest frontend and backend scenarios that match documentation changes.
   - Keep scenario IDs stable while replacing `TODO` placeholders with concrete Given/When/Then expectations and related files.
   - Align recommended sources and checklists with the guidance you added across docs and agents.
...
- `agents/index.json` highlights the correct roster, responsibilities, and instructions for the updated playbooks.
- Agent playbooks list accurate responsibilities, best practices, and pointer links to the refreshed docs.
- Changes are self-contained, well-formatted Markdown/JSON, and reference any new external resources introduced.

## Deliverables
- Updated Markdown and JSON files committed to the repository.
